...
# **About**
This library aims to provide a modular and robust RPC library for interacting with the Chia blockchain and directly interfacing with it via Java.
...
- Provide a modular/granular library implementation to allow for various levels of use.
- Provide a means to load self signed certificates via their .crt & .key files into a keystore.
- Provide a RPCClient wrapper around Apache Closeable HTTP Client for making direct requests to redefined endpoints usings a config. containing CertPairStore(Keystore) containing loaded chia certificates for authentication.
- Provide API classes that provide a higher level implementation of methods for making RPC request.
- Provide the option to manual handle response json.
- Provide immutable java records that correspond to the schemas of the response json.
- Insure immutability of all response data.
...
Using the above outlined features the library can be used simply to load self signed certificate/key pairs (which can be a challenge with java), be used simply as a RPCClient to make a process requests/response json manually or be used as a full abstraction around making RPC calls with requests returned as predefined immutable java records that map 1:1 with response data. Due to the nature of blockchains being immutable, and immutability being generally preferable when possible, all data relating to responses is contained in immutable records, and all collections that returned are also wrapped immutably. As such this library can provide a solid backbone for interfacing via RPC in a simple, safe and provide high performance processing of RPC requests across multiple nodes/wallets.
...
Currently the library provides implementations for all endpoints ***except for the Data Layer*** for the chia client as of 1.7. The library is still in a beta state, all implemented endpoints except for the ones introduced in version 1.7 of the client (Did not have a node on 1.7 when creating, but the new endpoints are implemented, just not tested) have been tested and should function as expected, but due to the nature of software development and the complexity of some of the endpoints and there return data, some latent bugs are to be expected. THe library will be updated and maintained as I use it in future projects, and I will try to keep it up to date as possible, but I cannot guarantee consistent full coverage of all endpoints. ***If any bugs are found please create an issue and I will investigate and fix them when time permits.*** I wll try my best to keep the every thing updated and fix any issues, but I am only one person. I tried to architect the library to be fairly simple, and easy to maintainable/contribute to. ***Any contributions are welcome, even if it is just something little.***
...
- Add JavaDocs mirror RPC documentation
- Add Unit Tests
- Implement BLS functionalities, or include from an existing library (https://github.com/chiachat/kbls is a good existing one)

<br>

When writing a didn't have any experience with with using the data layer and didn't implement any of those endpoints, this is something I plan to do in the future, or if anyone has experience with the datalayer and runs one would like to contribute to these that would be great. I also plan on adding JavaDocs to better document where builder methods are used and to mirror the details from the official documentation so there is no need to reference it and it can be read in the IDE when called the associated methods.

Unit tests are nots the easiest to implement due to the complexity of some of the parameters and returns, but still would be a great benefits for regression testing, proper validation, and will be indispensible for endpoint changes. There was a lot of ground to cover when writing all of the endpoint methods, so I didn't implement any unit tests while writing. I did test all the endpoints while implementing and manually validated them, but this not a true or maintainable metric to go by.

Somethings like fully crafting spendbundles would benefit from BLS functionality mainly aggregation, and it would also be nice to provide the ability to do puzzlehash <-> address conversion. KBLS(https://github.com/chiachat/kbls ) is a nice java library for this that I have used before, and likely will be implemented on the next release. I wanted to keep the first release simple so I didn't add it into this release. But the goal is to have the library provide all functionality needed to interface with the blockchain, and BLS signature aggregation is needed for this.
...
Before using the library a few initialization steps must be taken to define the endpoint URI of your node if remote, override ports if needed, and load your certificates for authentication of RPC requests. This config is then passed to the HTTP client that is used to make your requests.
...
A builder method is provide to make initialization via code the process of doing so is as follows:

* *The library make use of enums to aid useability, the builder for NodeConfig uses the enum class ChiaService, to define what      
...
The second step of readying the library for use is initializing the RPCClient, rpc client is a wrapper around``` org.apache.http.impl.client.CloseableHttpClient```. This wrapper provides the ability to initialize it with your NodeConfig, and then pass it to one or more API classes for making requests, it can be shared among several API classes that use the same node address and will pass requests made via those classes the the node defined in the config with proper authentication (as long as the certs exist and are valid).
...
You can now make requests directly to the node defined in your NodeConfig directly through the API object you created. Any endpoint provided by a service has an associated member method in the related API class, the the only difference being that the method name is in camelCase instead of snake_case.
...
// Or if you prefer not to declare the full generic type, since you will likely be grabbing the encapsulated object later
...
All requests return an instance of the generic record class APIResponse, this employs a optional generic "data" field will return either an object corresponding to the schema of the endpoints response, a built-in type in the case of single return values, or  ```Optional.empty``` in the case of a failed request, or if the request returns no values.

Optional was chosen here as to be able to properly handle the lack of a value on some responses in the context of the generic response class. The rational for usings a generic return class and an optional data value was to provide robust support for more functional modern java approaches and stream compatibility.
...
**endpoint:** An enum of the endpoint that the request was made to, can be used to filter response of the same type, that may have came from different endpoints.
...
***In some cases the data object may also contain redundant mappings of the success and error fields, but these can be ignored and were done for ease of testing where fail on unknown properties was used for debugging. These can be ignored, its best to use the top level fields of the ApiResponse class.***
...
Complex response data is represented by immutable record classes containing a 1:1 mapping of the response json, but with the field names in camelCase as is the naming convention with java. In the case of more complex queries there will be nested schemas. Object representations of response data where designed with modularity and reuse in mind to avoid redundant code.

In the case of single return values, the data object of ApiResponse will be just a single built-in types, it most cases this is a Integer value(fees, heights etc.), or a String(block headers, addresses, etc.)

In some cases where a list of types or values are returned, the return list/map is return in an immutable wrapper.

In the absence of value the data field will contain Optional.empty, as such before accessing the value a condition check must occur to avoid the chance of exceptions, while it most cases check for the request success will suffice, for best practice its best to always make sure the value is not empty.
...
For every API endpoint two request method, the "default" method that will return a parsed data object, and a secondary ``AsBytes`` method that will return the request exactly as received from the endpoint as an array of bytes. This secondary method can be used if you would prefer to parse the json with a different library and/or work directly with the json itself without having it return inside a data object.
...
If not wanting to use the build in API classes for requests, the RPCClient can be used to make request directly. This will return an array of bytes and allows full control of handling requests and the associated json. All the API classes use a jackson singleton wrapper for request, due to lazy initialization if these classes are not instanced and used, there is no need for the jackson dependency.

To make a request directly with RPCClient follow the initialization steps outlined above, without instancing any APIClasses. This RPCClient can then be used to make requests to the node defined in the NodeConfig.

Using the ```makeRequest(Request req)``` method of RPCClient a ```Request``` Object can be passed containing the data required for the request which will then be dispatched to the address in the NoneConfig used during the construction of the RPCClient.
...
public class Request {
    ...
    // Allows for overridding the service, this will be needed if using one of the shared endpoints
...
Any of the API endpoints that directly take an object node, have an associated builder method that can be found in ```io.mindspice.util.ChiaUtils```. These will later be documented in the JavaDocs for the project, but currently are not, which can be slightly confusing and was an oversight. JavaDocs for all the the API methods, will be added soon and should clear things up more. ***When encountering a method that takes a parameter of type JsonNode, the parameter name is the name of the builder you should use.*** These are used for endpoints that have a lot of parameters and/or optional parameters. If a method take an JsonNode (The builders actual return an ObjectNode, but these are interchangeable in this context) parameter, then there is a related builder method. There is also a builder method for crafting spendbundles.
...
There are also some methods to easily get a single field JsonNode ```newSingleNode``` and get empty nodes when need ```newEmptyNode``` and to merge two json structures.
